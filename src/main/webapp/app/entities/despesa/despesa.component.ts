...
export class DespesaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.despesaService
      ...
      .subscribe(
        (res: HttpResponse<IDespesa[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IDespesa): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInDespesas(): void {
    this.eventSubscriber = this.eventManager.subscribe('despesaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IDespesa[] | null, headers: HttpHeaders, page: number): void {
