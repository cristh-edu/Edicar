...
export class VendaService {
  ...
  protected convertDateFromClient(venda: IVenda): IVenda {
    const copy: IVenda = Object.assign({}, venda, {
      dtCompra: venda.dtCompra && venda.dtCompra.isValid() ? venda.dtCompra.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((venda: IVenda) => {
        venda.dtCompra = venda.dtCompra ? moment(venda.dtCompra) : undefined;
      });
    }
    return res;
  }
